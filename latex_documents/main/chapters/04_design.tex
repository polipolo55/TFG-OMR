...
\todo{Chapter lacks a \emph{Staff detection} design section: the pipeline figure shows a ``Staff detect'' block but no subsection explains the chosen approach (heuristic / YOLO / RT-DETR), its alternatives, or its accuracy target. Major omission for a five-stage pipeline.}
...
Figure~\ref{fig:pipeline-design} summarises the data flow.\todo{Use \Cref{fig:pipeline-design} mid-sentence instead of Figure~\ref; apply consistently throughout the chapter.}\todo{Inter-stage contracts are described in prose but never given as a typed schema (e.g.\ image dims, dtype, JSON shape). Add an explicit interface table or per-stage I/O contract block.}
...
\begin{tikzpicture}[
  box/.style={
    ...
    align=center, font=\footnotesize, text width=1.7cm,
    inner sep=2pt
  },
  arr/.style={->, thick, >=stealth},
  node distance=0.45cm
]
  %% ── Top row: input → preprocess → staff detect ──────────────────────
  \node[box, fill=blue!8]                          (in)   {PDF /\\image};
  \node[box, fill=blue!8,   right=0.55cm of in]    (pre)  {Preprocess};
  \node[box, fill=blue!8,   right=0.55cm of pre]   (det)  {Staff\\detect};

  %% ── Music stream (top branch) ───────────────────────────────────────
  \node[box, fill=orange!18, right=0.85cm of det]  (mcrn) {Music\\CRNN};
  \node[box, fill=orange!18, right=0.55cm of mcrn] (mctc) {CTC\\decode};

  %% ── Chord stream (bottom branch) ────────────────────────────────────
  \node[box, fill=teal!18,   below=0.9cm of mcrn]  (ccrn) {Chord\\CRNN};
  \node[box, fill=teal!18,   right=0.55cm of ccrn] (cctc) {CTC\\decode};

  %% ── Merge: grammar fix → JSON ───────────────────────────────────────
  \node[box, fill=violet!18, right=0.85cm of mctc, yshift=-0.75cm] (gf) {Grammar\\fix};
  \node[box, fill=blue!8,    right=0.55cm of gf]   (out) {JSON};

  %% ── Arrows ──────────────────────────────────────────────────────────
  \draw[arr] (in)  -- (pre);
  \draw[arr] (pre) -- (det);

  %% fork: staff detect → music CRNN (top) and chord CRNN (bottom)
  \draw[arr] (det.east) -- ++(0.25,0) |- (mcrn.west);
  \draw[arr] (det.east) -- ++(0.25,0) |- (ccrn.west);

  \draw[arr] (mcrn) -- (mctc);
  \draw[arr] (ccrn) -- (cctc);

  %% merge: both CTC decoders → grammar fix
  \draw[arr] (mctc.east) -- ++(0.25,0) |- (gf.west);
  \draw[arr] (cctc.east) -- ++(0.25,0) |- (gf.west);

  \draw[arr] (gf) -- (out);
\end{tikzpicture}
\caption[Five-stage inference pipeline.]{%
High-level data flow of the five-stage inference pipeline.
A preprocessing front-end normalises the input page; a staff detector
emits a staff image and a chord-strip image for every detected system;
two independent CRNN--CTC recognisers process the two streams in
parallel; a deterministic grammar fixer repairs structural defects in
the music stream; and the merged result is serialised as JSON.
...
\todo{Missing \emph{Alternatives considered} subsection for the pipeline decomposition: name $\geq$2 rejected designs (e.g.\ end-to-end full-page transformer; SMT++ tree-decoder) with reasons. FIB rubric requirement.}
...
expose the model to both beamed and unbeamed groupings.\todo{Concrete probabilities (``equal probability'', ``half of samples'', ``six-element range'', ``small fraction'') are implementation details. Either keep as ranges with justification or move exact values to Ch.5.}
...
\todo{Missing \emph{Alternatives considered} for augmentation strategy: e.g.\ GAN-based domain transfer, CycleGAN scan-style, no augmentation + larger model. Justify rejection of each.}
...
\Cref{sec:soa-representations} surveyed the available symbolic
...
linearisation logic required at the model boundary.\todo{Missing explicit \emph{Alternatives considered} subsection: MusicXML, PrIMuS agnostic/semantic, \code{**kern}, ABC each need a one-line rejection reason here, not just a forward pointer to Ch.\,2.}
...
table, optimiser state---live in \Cref{ch:impl}.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[
    box/.style={
      ...
      align=center, font=\small, text width=1.8cm,
      inner sep=3pt
    },
    arr/.style={->, thick, >=stealth},
    ...
    \node[box, fill=blue!8]                          (img)  {Staff\\strip\\image};
    \node[box, fill=orange!18, right=0.55cm of img]  (cnn)  {ResNet18\\backbone};
    \node[box, fill=orange!18, right=0.55cm of cnn]  (col)  {Collapse\\height};
    \node[box, fill=teal!18,   right=0.55cm of col]  (lstm) {BiLSTM\\$2\times 256$};
    \node[box, fill=violet!18, right=0.55cm of lstm] (lin)  {Linear\\+\,softmax};
    \node[box, fill=red!10,    right=0.55cm of lin]  (ctc)  {CTC\\decoder};
    \node[box, fill=blue!8,    right=0.55cm of ctc]  (out)  {Token\\sequence};
    ...
    \node[ann, below=0.2cm of cnn]  {$128\!\times\! W \to$\\$1\!\times\! W/4$ map};
    \node[ann, below=0.2cm of col]  {width-indexed\\frames};
    \node[ann, below=0.2cm of lstm] {2 layers,\\512-dim output};
    \node[ann, below=0.2cm of lin]  {$|V|$ = vocab\\size};
    \node[ann, below=0.2cm of ctc]  {blank-collapsed\\alignment};
  \end{tikzpicture}
  \caption[CRNN--CTC melody recogniser design.]{%
  Block diagram of the CRNN--CTC melody recogniser.
  A ResNet18 backbone with asymmetric strides compresses the input
  staff strip from $128\times W$ down to a $1\times W/4$ feature map;
  the height-collapsed sequence of feature columns is consumed
  left-to-right by a two-layer bidirectional LSTM (256 hidden units per
  direction, 512-dim concatenated output); a linear projection produces
  a per-frame distribution over the vocabulary; and CTC decoding
  collapses the blank-padded alignment into a flat LMX token sequence.}
  \label{fig:crnn-arch}
\end{figure}
...
image is wide.
This matches the data assumptions of CTC~\cite{gravesConnectionistTemporal2006} exactly:
...
(\Cref{sec:soa-transformer}).\todo{This is the chapter that should carry the CRNN--CTC technical case. Promote to an explicit \emph{Alternatives considered} subsection naming $\geq$2 rejected designs (e.g.\ encoder--decoder transformer/TrOMR, tree-decoder SMT++) with reasons, rather than deferring to Ch.\,2.}

\subsection{Backbone choice}

The default backbone is a modified
ResNet18~\cite{heDeepResidualLearning2016} with asymmetric strides that compress the height
...
(see \Cref{ch:mgmt}).\todo{Backbone choice needs an explicit \emph{Alternatives considered}: ResNet34/50, EfficientNet, MobileNet, ViT/Swin patches---each with a one-line reason for rejection.}
...
parameter count modest.\todo{Exact hyperparameters (``256 hidden units'', ``two layers'', \SI{40}{\percent} header strip, \SI{10}{\percent} splits, etc.) belong in Ch.\,5. Here state the design rationale (``small enough to fit on one consumer GPU''); move concrete values to implementation.}
A single-layer LSTM was found to under-fit on dense passages
(see ablations in \Cref{ch:results}); a three-layer LSTM gave no
...
\Cref{sec:design-model}; its vocabulary is the LMX token set.
...
this is a deliberate simplicity choice.\todo{Two-stream split is a major decision but the rejected alternative (single multi-task network with shared backbone, or full-page transformer) is only sketched. Add an \emph{Alternatives considered} listing $\geq$2 with reasons.}
...
multiplier.\todo{``Training loader'' / ``configurable multiplier'' drifts into Ch.\,5 territory. Ch.\,4 should state the design choice (oversample rare-token samples instead of class-weighted loss) and \emph{why}; leave the loader mechanism for the implementation chapter.}
...
limited gain for a vocabulary as small and as well-structured as LMX.\todo{Grammar fixer needs an explicit \emph{Alternatives considered} block listing $\geq$2 rejected designs (e.g.\ constrained CTC decoding with grammar-aware beam, FST-based decoder, learned seq2seq corrector) with reasons---not only a sentence on grammar-aware loss.}
...
\todo{No quantitative acceptance / success target. State the SER threshold (e.g.\ ``target SER\,$<$\,X\,\% on clean test, $<$\,Y\,\% on real scans'') that defines whether the design has succeeded; otherwise Ch.\,6 has nothing to compare against.}
