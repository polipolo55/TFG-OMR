...
amb la \term{Taxa d'Error de Símbols} (SER), assolint un \textbf{1{,}19\%} sobre el
conjunt de prova sintètic net i un \textbf{1{,}28\%} sobre la versió simulada com a
escanejada, i un ajust fi sobre pàgines reals del Real Book millora el rendiment
...
con la \term{Tasa de Error de Símbolos} (SER), alcanzando un \textbf{1{,}19\%} sobre el
conjunto de prueba sintético limpio y un \textbf{1{,}28\%} sobre la versión simulada
como escaneada, y un ajuste fino sobre páginas reales del Real Book mejora
...
Evaluated using the \term{Symbol Error Rate} (SER), the model achieves \textbf{1.19\%}
on the clean synthetic test set and \textbf{1.28\%} on its scan-simulated counterpart,
and a fine-tuning stage on real Real Book pages further improves performance on
