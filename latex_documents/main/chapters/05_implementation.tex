...
trains on \num{87678} PrIMuS samples (\num{86113} after the domain
filters of \Cref{sec:design-data}) plus their augmented copies, with
a vocabulary of \num{273} LMX tokens
(\todo{measure: exact post-filter sample count and vocab size from
\code{data/vocab/primus\_lmx.txt}}), and converges in
\num{37}~epochs of about
\todo{measure: epoch wall-clock on RTX~3060 with batch~16, AMP enabled}
each on the target hardware.
The final checkpoint occupies approximately
\todo{measure: size of \code{models/latest/best\_model.pt} in MB}.
...
The data pipeline of \Cref{sec:design-data} is implemented as four CLI
subcommands run in sequence: \code{render}, \code{convert},
\code{augment}, and \code{vocab}.
...
\begin{figure}[ht]
  ...
  in LilyJAZZ, converted to LMX labels, augmented to simulate scanned
  conditions, and finally indexed into a vocabulary file.}
...
\code{batch\_size}               & 16                 & peak VRAM $\approx$ \SI{8.5}{\giga\byte} on RTX~3060\todo{measure: peak VRAM at batch 16 with AMP enabled} \\ \tfgrowsep
...
Training uses the same optimiser, scheduler, and CTC configuration as
the melody network (AdamW with peak $\text{lr}=1{\times}10^{-3}$,
OneCycleLR, gradient clip $5.0$).
\todo{Add concrete chord-network hyperparameters: number of epochs on
the synthetic-only phase, batch size, fine-tuning learning rate,
real/synthetic mixing weights schedule — currently only the
architecture is detailed.  Defaults are in \code{chord\_train.py}.}
...
input shape\todo{measure: exact parameter count and FLOPs of the
modified ResNet18 at $(1, 1, 128, 512)$ via \texttt{torchinfo} or
\texttt{fvcore}}.
...
\todo{measure: exact \texttt{vocab\_size} after filter, currently
$\approx 270$}, and the CTC forward--backward shrinks proportionally.
...
The four offline stages (\code{render}, \code{convert}, \code{augment},
\code{vocab}) are CPU-bound: LilyPond invocation, image augmentation
...
\SI{8.5}{\giga\byte} at \code{batch\_size=16}\todo{measure: peak VRAM
with AMP on vs off at batch sizes 16, 24, 32}, leaving headroom for
...
pinned memory buffers, and \code{persistent\_workers} keeps the
workers alive between epochs to amortise the
process-spawn cost\todo{measure: per-epoch wall-clock with
\code{persistent\_workers=True} vs \code{False}; reported anecdotally
as a noticeable improvement during development}.
...
\todo{Pin and document the LilyPond version and the LilyJAZZ commit
hash used for the rendered corpus; renders are part of the
reproducibility chain and a future LilyPond release could subtly
shift output pixels.}

\paragraph{Random-seed strategy.}
\todo{Document the random-seed policy explicitly: which subsystems
seed PyTorch / NumPy / Python at startup (\code{torch.manual\_seed},
\code{numpy.random.seed}, \code{random.seed}); whether
\code{torch.use\_deterministic\_algorithms} and the cuDNN benchmark
flag are toggled; and which CLI flags (e.g.\ \code{--seed} for
\code{augment}) propagate to the worker pool.  At present
\code{dataset.py::make\_splits} uses a \code{torch.Generator} seeded
explicitly, and the augmentation seed is exposed via the
\code{--seed} CLI flag, but a global per-run seed for the training
loop should be added and documented here.}
