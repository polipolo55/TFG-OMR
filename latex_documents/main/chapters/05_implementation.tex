...
\code{lilypond} subprocess at the requested DPI. A single value fixes
the render resolution; the full \code{pipeline} and \code{pipeline-train}
paths pass \SI{200}{DPI}, \SI{250}{DPI}, and \SI{300}{DPI}, which makes
the renderer sample one of the three per score.
...
order: morphological ink dilation (0, 1, or 2 dilation iterations),
staff-line thickness variation ($p=0.40$), paper-tone remapping to
$[28, 245]$, a mutually exclusive lighting roll (approximately
$p=0.50$ radial vignette, $p=0.35$ directional uneven illumination,
and $p=0.15$ neither), and halftone banding ($p=0.25$).
...
To widen visual diversity further, the \code{augment} stage is run a
second time with a different seed to produce two extra scan variants
per sample (\code{scanned\_variant\_dirs}); during training each staff
draws uniformly among its base scanned render and these variants, while
validation and test always use the single base render.
Because the variants are keyed by sample identifier and the train/test
split is over identifiers, no variant can cross the split boundary.
This is the train-only diversity mechanism; it replaces an earlier
continuation-staff scheme and carries no inference-time cost.
...
\subsection{Warm-start fine-tuning}
\label{sec:impl-finetune}

The second training phase of \Cref{sec:design-training} reuses the same
loop with two changes.  The \code{--init-weights} flag loads the model
weights from a converged synthetic checkpoint and discards its optimiser
and scheduler state, so training opens a fresh OneCycle schedule instead
of resuming the decayed tail of the original run; this is what separates
a fine-tune from the crash-recovery \code{--resume} path, which restores
the full optimiser state and continues the same run in place.  The peak
learning rate is lowered to $1{\times}10^{-4}$, against the
$1{\times}10^{-3}$ of phase one, so the small real set shifts the feature
distribution without overwriting the synthetic representations.  Real
staves enter through \code{--finetune-data-dir}, which appends them to
the training split only (\Cref{sec:design-training}); the annotation
itself uses the model-assisted music-strip UI served at
\code{/music-labeler} (\Cref{sec:impl-api}), the melody counterpart of
the chord labeller below.

% ─────────────────────────────────────────────────────────────────────────────
\section{Chord Recognition Implementation}
...
chords each, in the LilyJAZZ font, with its own DPI jitter range
(\SIrange{140}{260}{DPI}) chosen for the effective size of cropped
chord text. It then applies an \code{albumentations} pipeline tuned for
...
The default configuration produces \num{30000} training strips and
\num{2000} validation strips; the chord training script consumes them
via a CSV manifest.
...
The synthetic pre-training phase uses AdamW with a peak learning rate
...
from overwriting chord representations built during pre-training.
...
real strips carry weight $1.0$, synthetic strips weight $0.5$ (the
...
CRNN's argmax frames against \code{min\_mean\_logprob}.
Thresholds live in \code{models/staff\_reject/thresholds.json}.
The four geometric thresholds are fitted by the \code{calibrate-reject}
CLI subcommand (\Cref{sec:impl-cli}) on a labelled fixture set and depend
only on staff geometry, not on the trained weights; the
\code{min\_mean\_logprob} gate, by contrast, is a hand-tuned,
music-preserving value that \code{calibrate-reject} leaves at a
conservative default, so it is re-tuned by hand only if a retrained
recogniser begins rejecting genuine staves.
...
batched, and decoded.
When the network meets clutter absent from the synthetic training set it
emits \code{<unk>} tokens, which are treated as word separators rather
than as character errors.
...
extracted chord list (\Cref{fig:web-ui}).
The UI functions as a debugging aid and demonstration interface rather
than a primary deliverable.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\textwidth]{figures/web_example}
    \caption{The web interface wrapping the inference pipeline, shown
    transcribing the genuine \textit{Satin Doll} Real Book scan.
    A PDF or image is uploaded through the drop zone at the top; the
    pipeline then returns the rendered page below with each detected
    staff system outlined, and lists the recognised chord symbols and
    LMX token sequence for every system.
    On this page the service detects nine staff systems, rejects two
    (the title band and the bottom credit line) at the staff gate, and
    transcribes the remaining seven; the same scan is shown with the
    reject-gate decisions annotated in \Cref{fig:realpage-annotated}.}
    \label{fig:web-ui}
\end{figure}
...
a threaded variant ran slower in development because of GIL
...
PDF DPI (\code{OMR\_PDF\_DPI}) & Linear: image area scales as DPI$^2$. & Linear in image area. & Higher DPI preserves more staff detail up to the \SI{300}{DPI} operating point; beyond that pre-processing cost grows quickly. \\ \tfgrowsep
Pre-CRNN reject gate & Improves throughput by skipping non-music strips' CRNN call. & Reduces the average latency per page. & Trade-off between false-positive rejection of valid staves and false-negative acceptance of text; geometric thresholds are calibrated on labelled fixtures, while the CTC confidence gate is hand-tuned. \\
\bottomrule
\end{tabularx}
...
\SI{300}{DPI} rasterisation, and the rejection thresholds in
\code{models/staff\_reject/thresholds.json}: this is the operating
...
The \code{make\_splits} function builds the filtered sample list in a
stable order and applies a seeded random permutation (\code{seed=42}),
holding out \SI{10}{\percent} for validation and \SI{10}{\percent} for
test (the remaining \SI{80}{\percent} for training).
The same source tree and seed produce the same split across runs, so
any difference between two evaluation runs comes from training, not
from the split.
...
\code{model\_state\_dict}, \code{optimiser\_state\_dict},
