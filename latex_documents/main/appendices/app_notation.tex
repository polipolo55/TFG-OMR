...
An accidental holds for that pitch, at that octave, for the rest of the measure in which it appears, then lapses.
...
Reading \texttt{C7b9}, for example: root C, quality \texttt{7} (dominant
seventh), alteration \texttt{b9}; and \texttt{C/E} is a C major triad voiced
over an E in the bass.

The Real Book uses a stylised dialect of this notation: minor is written with
a hyphen (\texttt{G-7}, not \texttt{Gm7}), major seventh with \texttt{maj}
(\texttt{Fmaj7}), diminished with \texttt{dim}, augmented with \texttt{+}, and
half-diminished most often as \texttt{-7b5} (with \texttt{m7b5} and the
\texttt{ø} glyph as rarer variants).  The chord recogniser of this thesis
targets that dialect; \Cref{sec:design-twostream} describes how the variants
are handled.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{} l l l @{}}
        ...
        Quality                  & chord-type abbreviation               & \texttt{maj7}, \texttt{-7}, \texttt{7}, \texttt{-7b5}, \texttt{dim7} \\
        Extensions / alterations & added or altered chord tones          & \texttt{\#9}, \texttt{b9}, \texttt{13} \\
        Slash bass               & non-root note placed in the bass      & \texttt{/E} in \texttt{C/E} \\
        \bottomrule
    \end{tabular}
\end{table}

Because chord symbols are text rather than staff glyphs, recovering them is a
character-sequence recognition problem: analogous to OCR, but implemented here
with a dedicated CRNN--CTC network rather than an off-the-shelf OCR engine.
This split motivates the two-stream architecture of \Cref{ch:design}.

\section{The Lead-Sheet Format}

A jazz lead sheet assembles the elements above into a minimal score: one or more
\term{systems}, each a single-voice treble-clef staff carrying the melody (and opening
with a clef, key signature, and time signature), chord symbols above the staff, and
...
\begin{figure}[htbp]
    ...
    The annotations mark \textbf{(1)} header metadata, \textbf{(2)} chord
    symbol regions targeted by the chord-recognition stream, and
    \textbf{(3)} monophonic staff systems targeted by the CRNN--CTC model.}
