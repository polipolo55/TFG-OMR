...
that produced a key and time signature (`first_keyed_idx`). For all subsequent
non-rejected staves, it prepends the matching prerendered header template and
re-runs the CRNN:

```
raw continuation staff:      [notes only, no clef/key/time]
after header injection:      [clef][key][time][notes...]
```

The CRNN then sees a full-header staff matching its training data. Grammar fix
pass 2 runs on the corrected predictions. If no time signature was detected at
all (e.g. first staff is title text and was rejected), the pipeline falls back
to injecting a 4/4 default.
