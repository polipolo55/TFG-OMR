...
keep the system within the compute budget of a single consumer GPU.
...
file type. Three structural rules hold it together. The music-notation
mapping tables (clef IDs, key-signature encodings, duration tokens)
live in a single module, \code{CRNN\_CTC/lilypond\_render.py}, imported
wherever they are needed, which keeps the rendering pipeline and the
label converter from drifting out of sync. The inference pipeline
(\code{omr\_pipeline/}) depends on the training code only through the
\code{Vocabulary} and \code{CRNN} classes, so it can be deployed
without the training tree. And \code{cli.py} is the only entry point:
every multi-step operation is a subcommand sharing the same flag names
and defaults.
...
src/
  ...
  CRNN_CTC/         model, training, vocab, evaluation,
                    and chord-network code
  data_processing/  dataset generation and augmentation
  omr_pipeline/     inference pipeline (preprocess, staff
                    detect, reject gate, CRNN, chord OCR,
                    grammar fixer)
...
weights.
The key values are: batch size~16 (peak VRAM $\approx$ \SI{8.5}{\giga\byte}
on the RTX~3060), peak learning rate $1{\times}10^{-3}$ with OneCycleLR,
weight decay $1{\times}10^{-4}$, gradient clip $5.0$, and early stopping
with a patience of~12 epochs.
The complete parameter list, including architecture sizes and split
fractions, is in \Cref{tab:hyperparams-omr} and \Cref{tab:arch-omr}
of \Cref{app:hyperparams}.
Because the \code{Config} dataclass is part of the checkpoint contract,
adding, renaming, or removing a field is a backward-incompatible change
...
End-to-end per-page latency on the target RTX~3060 is reported in
\Cref{ch:results}.
...
Greedy is the runtime default for the CLI \code{evaluate} subcommand
and the API; beam search is opt-in. The accuracy gain and decode-cost
multiplier are reported in \Cref{ch:results}.
...
and a forward-pass cost of roughly \num{1.8}~GFLOP at the project's
input shape (estimates for the standard ResNet18; exact values vary
with the asymmetric stride modifications above). The choice over the
legacy VGG stack is argued in \Cref{sec:design-model}.
...
The end-to-end per-page wall time on the target RTX~3060 and the
