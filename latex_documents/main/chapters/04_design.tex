...
image is wide.
This matches the data assumptions of CTC exactly:
...
The default is a modified ResNet18 with asymmetric strides that compress
the height dimension aggressively while preserving width, leaving the
horizontal dimension as the CTC time axis.
The alternative is a five-block VGG-style stack kept for backward
compatibility with earlier checkpoints; it converges more slowly and is
no longer used for new experiments.
...
The convolutional features are fed to a stacked bidirectional LSTM.
...
\begin{itemize}
    ...
    \item normalise predictions outside the lead-sheet contract: any
          ...
          rejected;
    \item clamp predicted octaves to the in-domain range $[3, 6]$,
