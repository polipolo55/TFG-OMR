...
Every library was chosen as the smallest widely-used option compatible
with the design constraints; no component was chosen for novelty.
The full version-pinned list is in \Cref{tab:tech-stack}
...
change for existing checkpoints (Hard~Constraint~3 of the contributor
guide).
...
On a representative \num{8}-staff page the median end-to-end latency
on the target RTX~3060 is \SI{165}{\milli\second} (mean
\SI{166}{\milli\second} $\pm$ \SI{12}{\milli\second} over ten runs,
measured via \code{notebooks/02\_evaluate\_model.ipynb} §9).
...
\num{8}-staff page is approximately \SI{165}{\milli\second} (median
over ten runs at \SI{300}{DPI}, greedy decoding).

\paragraph{Cost breakdown across stages.}
On an \num{8}-staff page the stage costs are: preprocessing
(CLAHE $+$ deskew $+$ binarisation) \SI{26}{\milli\second},
morphological staff detection \SI{48}{\milli\second},
music CRNN $+$ greedy decode \SI{34}{\milli\second} for all staves in
batch, chord CRNN \SI{28}{\milli\second}, and grammar fixer
${<}$\SI{1}{\milli\second}.
Per strip, the music CRNN forward pass and greedy CTC decode together
cost approximately \SI{13}{\milli\second} at $W \approx 1\,500$~px
(median over 20 runs of a ten-strip batch at the training image height
of \SI{128}{px}).
The chord CRNN runs on a strip of half the staff height, so its
forward-pass cost is proportionally smaller; its total throughput is
dominated by the per-batch CUDA-kernel launch overhead.
The grammar fixer is a pure-Python state machine; its cost is
negligible at any batch size.
