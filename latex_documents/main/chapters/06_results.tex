...
Chapters~\ref{ch:design} and \ref{ch:impl}.
Section~\ref{sec:res-training} presents the training trajectory of the
synthetic-data baseline.
Section~\ref{sec:res-ser} reports aggregate Symbol Error Rate on the
held-out synthetic test split and analyses the distribution of
per-sample errors.
Section~\ref{sec:res-errors} decomposes the errors by edit operation
...
Section~\ref{sec:res-qualitative} shows qualitative examples drawn
from the perfect and near-median tails of the SER distribution.
Section~\ref{sec:res-domaingap} addresses the synthetic-to-real
...
The default configuration of Table~\ref{tab:hyperparams} was trained
...
domain filters of Section~\ref{sec:design-data}.
Figure~\ref{fig:training-curves} shows the trajectory of the CTC loss
...
\begin{figure}[htbp]
    ...
    plateaus near \num{0.06}, indicating a small but bounded
    overfit margin.
...
drawn from the union of clean and scan-augmented images) using greedy
CTC decoding.
Aggregate SER, computed as the sum of edit distances divided by the
sum of ground-truth lengths, is \SI{1.28}{\percent} on the scanned
test split and \SI{1.19}{\percent} on the clean test split.
...
Figure~\ref{fig:ser-distribution} shows the per-sample SER
distribution.
...
On this metric, the same checkpoint achieves $\approx \SI{0.18}{\percent}$
on the scanned split and $\approx \SI{0.11}{\percent}$ on the clean
split---roughly an order of magnitude lower than the aggregate SER.
The motivation for reporting both numbers becomes clear once the
error budget is decomposed by token category, the subject of the
next section.
...
Figure~\ref{fig:edit-dist} shows the distribution.
...
of the edit budget---an encouraging finding, because substitutions
are precisely the errors that affect musical \textit{content}.
...
Figure~\ref{fig:common-edits} shows the top-15 substitutions,
...
deterministic grammar fixer (Section~\ref{sec:design-grammar}) rather
...
Figure~\ref{fig:ser-vs-length} shows that the median per-sample SER
...
backbone (Section~\ref{sec:design-model}) is appropriate for the
...
print, and the model recovers the underlying notation cleanly in
both cases.

\todo{Add a worst-case example from the right tail of the SER
distribution, with a brief failure-mode caption explaining what the
model got wrong and why.}
...
Figure~\ref{fig:weight-dist} shows the distributions of the four
...
\todo{Quantitative zero-shot evaluation on the hand-annotated Real
Book set: SER on real scans \textit{before} fine-tuning, broken down
by the same token categories as the synthetic evaluation above.
A bar chart comparing the four major error categories on synthetic
versus real input would make the gap visible at a glance.}

\subsection{Fine-tuning results}

\todo{Quantitative comparison of the same model \textit{after}
fine-tuning on the small hand-labelled Real Book set: SER on the
same real test pages, with the synthetic-test SER reported alongside
as a non-regression check.
A second bar chart comparing pre-fine-tune SER, post-fine-tune SER
on real pages, and the synthetic-test SER would close the loop on
the principal experimental question of this thesis.}

\subsection{Qualitative inference on a complete Real Book page}

\todo{One full-page inference example: a Real Book scan with detected
staff bounding boxes overlaid, the predicted LMX token sequence
beneath each staff, and the recognised chord symbols above.
A figure of this kind functions as the project's headline
qualitative result.}
...
The synthetic-data baseline trained from scratch achieves an
aggregate Symbol Error Rate of \SI{1.28}{\percent} on the
scan-augmented test split and \SI{1.19}{\percent} on the clean
split, with $\approx \SI{71}{\percent}$ of samples transcribed
perfectly.
The residual error budget is dominated by structural tokens
(\code{measure} and the two tie tokens), which together account for
$\approx \SI{87}{\percent}$ of all edits;
pitch and octave errors are negligible.
The melodic Symbol Error Rate---measured on the same edit distance
after removing the structural tokens---is approximately
\SI{0.18}{\percent}, an order of magnitude lower than the
aggregate.
These numbers, combined with the deterministic grammar fixer that
reconstructs barline boundaries from the time signature, place the
synthetic baseline within the operating range required for
downstream musical applications.
...
in Section~\ref{sec:res-domaingap}, which will be completed as
