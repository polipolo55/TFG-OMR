...
Section~\ref{sec:sust-selfassessment} reports the self-assessment
that opened the project; Section~\ref{sec:sust-matrix} fills the
...
its anticipated useful life; Section~\ref{sec:sust-commitment}
records the ethical position and open-science commitments adopted;
and Section~\ref{sec:sust-final} revisits the assessment in light
...
Section~\ref{sec:mgmt-justification}.
...
The energy estimate of Section~\ref{sec:sust-environmental} is the
...
The comparison table in Section~\ref{sec:mgmt-justification}
...
Section~\ref{sec:sust-social} and the ethical-position statement
in Section~\ref{sec:sust-commitment}: both name specific
...
Table~\ref{tab:sustainability-matrix} summarises the
...
\begin{table}[h!]
    ...
    \begin{tabularx}{\linewidth}{>{\raggedright}p{2.3cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        ...
        Economic
            ...
            & Open-source release and fast local inference can reduce recurrent transcription cost compared with manual copying and commercial OMR tools. \\
        Environmental
            ...
            & One-time training cost is amortised over repeated inference; local execution reduces cloud dependency for common use cases. \\
        Social
...
(Section~\ref{sec:mgmt-budget}), dominated by specialised
...
correctly (Chapter~\ref{ch:results}).
...
First, the architecture choice (Section~\ref{sec:design-model})
...
Section~\ref{sec:mgmt-justification} makes this trade-off
explicit.
...
Chapter~\ref{ch:results} produces a much more concrete picture
...
(Section~\ref{sec:design-data}) means the model is not designed
...
accuracy reported in Chapter~\ref{ch:results} and would have to
...
Chapter~\ref{ch:results}, while small in aggregate, is not zero;
...
(Section~\ref{sec:impl-reproducibility}), and the trained model
...
The metric protocol used in Chapter~\ref{ch:results} is documented
...
(Sections~\ref{sec:scope} and \ref{sec:sust-social}).
...
\num{37} epochs (Section~\ref{sec:res-training}) and the full
